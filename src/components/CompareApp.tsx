...
export default function CompareApp() {
  ...
  // Use state + effect so server and initial client render match (avoids hydration mismatch)
  ...
  async function runFileDiff(left: FileInfo, right: FileInfo) {
    ...
    setView('diff');
    // After the diff view mounts, auto-navigate to the first diff
    if (diffs > 0) {
      setTimeout(() => {
        setCurrentDiff(0);
  ...
  async function runFolderDiff(left: DirInfo, right: DirInfo) {
    ...
    await tick();
    try {
      const items = await buildFolderItems(left.handle, right.handle);
  ...
  const showSync = mode === 'file' && !!leftFile && !!rightFile;

  return (
    <div className="flex flex-col h-screen overflow-hidden">
      <Toolbar
        mode={mode}
        onSetMode={m => { setMode(m); setView('welcome'); }}
        diffCount={diffCount}
